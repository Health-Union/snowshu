...
class BaseTargetAdapter(BaseSQLAdapter):
    ...
    def create_and_load_relation(self, relation: "Relation", data: Optional[pd.DataFrame], clone: bool = False) -> None:
        if relation.is_view:
    ...
    def load_data_into_relation(self, relation: Relation, data: Optional[pd.DataFrame], clone: bool = False) -> None:
        ...
        if data is None and relation.data.empty:
            logger.warning(
                "Both data and relation.data are empty for %s. " "Empty database, schema, and table will be created.",
                self.quoted_dot_notation(relation),
            )
            final_message = f"{self.quoted_dot_notation(relation)} created with no data."
        else:
            logger.info("Loading data into relation %s...", self.quoted_dot_notation(relation))
            final_message = f"Data loaded into relation {self.quoted_dot_notation(relation)}."
    ...
    def quoted_dot_notation(
        ...
        return ".".join([self.quoted(getattr(rel, relation)) for relation in ("database", "schema", "name")])

    def create_function_if_available(self, function: str, relations: Iterable["Relation"]) -> None:
        ...
        """
        try:
            functions_path = Path(__file__).resolve().parent / f"{self.name}_adapter" / "functions"
            ...
            for db, schema in unique_schemas:  # noqa pylint: disable=invalid-name
                ...
                conn = self.get_connection(database_override=database, schema_override=schema)
