...
class BruteForceSampling(BaseSampling):
    ...
    def __init__(
        self, probability: float = 0.10, min_sample_size: int = 1000, max_allowed_rows: int = MAX_ALLOWED_ROWS
    ):
    ...
    def prepare(self, relation: "Relation", source_adapter: "BaseSourceAdapter") -> None:
        ...
        """
        self.size = min(
            max(self.sample_size_method.size(relation.population_size), self.min_sample_size), self.max_allowed_rows
        )
