            ...
            for i,relation in enumerate(nx.algorithms.dag.topological_sort(executable.graph)):
                ...
                if executable.analyze:
                    ...
                    if relation.is_view:
                        ...
                        relation.sample_size="N/A"
                        try:
                            relation.view_ddl=executable.source_adapter.check_count_and_query(relation.compiled_query,MAX_ALLOWED_ROWS).iloc[0][0]
                        except Exception as e:
                            raise SystemError(f'Failed to extract DDL statement: {relation.compiled_query}')
                        logger.info(f'Successfully extracted DDL statement for view {relation.quoted_dot_notation}')
                    else:
                    ...
                    logger.info(f'Inserting relation {relation.quoted_dot_notation} into target...')
                    try:
                        executable.target_adapter.create_and_load_relation(relation)
                    except Exception as e:
                        raise SystemError(f'Failed to load relation {relation.quoted_dot_notation} into target: {e}')
