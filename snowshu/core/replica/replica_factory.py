...
class ReplicaFactory:
    ...
    def _load_full_catalog(self) -> None:
        ...
        logger.info(
            f'Done assessing catalog. Found a total of {len(self.full_catalog)} relations from the source in {duration(start_timer)}.')
    ...
    def _load_credentials(self, credentials_path: str,
        ...
        requested_profiles = dict(
            source=source_profile, storage=storage_profile)

        for section in ('source', 'storage',):
            sections = f"{section}s"  # pluralize ;)
            logger.debug(
                f'loading {sections} profile {requested_profiles[section]}...')
    ...
    def _fetch_adapter(self, adapter_type: str, adapter_name: str):
        ...
        try:
            classnamed = ''.join([part.capitalize()
                                  for part in adapter_name.split('_')] + ['Adapter'])

            self.__dict__[f"{adapter_type}_adapter"] = adapters.__dict__[
                f"{classnamed}"]()
            logger.debug(f'{adapter_type} adapter set to {classnamed}.')
        except KeyError as e:
            logger.error(
                f"failed to load config; {adapter_name} is not a valid {adapter_type} adapter.{e}")
            raise e
