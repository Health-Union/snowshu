...
class RuntimeSourceCompiler:
    ...
    def compile_queries_for_relation(
        relation: Relation,  # pylint: disable=too-many-locals, too-many-branches
        dag: networkx.Graph,
                    ...
                    if edge["direction"] == "bidirectional":
                        predicates.append(
                            source_adapter.upstream_constraint_statement(
                                child, edge["remote_attribute"], edge["local_attribute"]
                            )
                        )
                        ...
                        if "local_type_attribute" in edge:
                            ...
                            polymorphic_predicates.append(
                                source_adapter.predicate_constraint_statement(
                                    parent, analyze, edge["local_attribute"], edge["remote_attribute"]
                                )
                            )
                    else:
                        predicates.append(
