...
class SnowShuGraph:
    ...
    def build_graph(self, configs: Configuration) -> None:
        ...
        if not networkx.algorithms.is_directed_acyclic_graph(self.graph):
            ...
            logger.error(
            f"""The dependency graph generated by the given specified relations yields a cyclic graph.
            \r Cyclic dependency found in the following relations: \n {graph_cycles}""")
            raise ValueError(
                f'The graph created by the specified trail path is not directed (circular reference detected): \n{cycle_names}')
