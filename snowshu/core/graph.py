...
from snowshu.core.models.relation import Relation, single_full_pattern_match
...
class SnowShuGraph:
    ...
    @staticmethod
    def catalog_difference(
        source_graph: Union["SnowShuGraph", networkx.Graph], target_catalog: Union[tuple, list, set]
    ) -> networkx.Graph:
    ...
    def build_graph(self, configs: Configuration) -> None:
        ...
        catalog = configs.source_profile.adapter.build_catalog(
            patterns=self.build_sum_patterns_from_configs(configs), thread_workers=configs.threads
        )
        ...
        if not networkx.algorithms.is_directed_acyclic_graph(self.graph):
            ...
            if filename:
                logger.error(
                    ...
                    \n\t\033[0;36m%s.png \033[1;37m and \033[0;36m%s.graphml",
                    message,
                    filename,
                    filename,
                )
    ...
    @staticmethod
    def _set_overriding_params_for_node(relation: Relation, configs: Configuration) -> Relation:
    ...
    def _apply_specifications(  # noqa pylint: disable=too-many-locals
        configs: Configuration, graph: networkx.MultiDiGraph, available_nodes: Set[Relation]
        ...
        for relation in configs.specified_relations:
            ...
            relation_pattern_dict = dict(
                name=relation.relation_pattern, database=relation.database_pattern, schema=relation.schema_pattern
            )
            ...
            for relationship in relationship_dicts:
                ...
                if wildcard_attrs:
                    ...
                    for _, downstream_partition in wildcard_partitions.items():
                        ...
                        graph = SnowShuGraph._process_downstream_relation_set(
                            relationship, downstream_partition, graph, available_nodes
                        )
                # no wildcards present in relationship definition
                else:
                    graph = SnowShuGraph._process_downstream_relation_set(
                        relationship, downstream_relations, graph, available_nodes
                    )
    ...
    def _process_downstream_relation_set(
        ...
        graph: networkx.MultiDiGraph,
        full_relation_set: Set[Relation],
    ) -> networkx.Graph:
        """Adds the appropriate edges to the graph for the given relationship"""
        # pylint: disable-msg=too-many-locals
        # find any of the upstream relations
        upstream_relations = set(filter(lambda x: single_full_pattern_match(x, relationship), full_relation_set))
        ...
        catalog_dict = dict(
            map(lambda x: (x.dot_notation, dict(map(lambda a: (a.name, a.data_type), x.attributes))), full_relation_set)
        )
        ...
        for downstream_relation in downstream_set:
            for upstream_relation in upstream_without_downstream:
                ...
                attributes = dict(
                    filter(
                        lambda item: item[0] in ["remote_attribute", "local_attribute"],
                ...
                graph.add_edge(upstream_relation, downstream_relation, **relationship["edge_attributes"])
    ...
    def get_connected_subgraphs(self) -> tuple:
        ...
        dags = [
            networkx.induced_subgraph(self.graph, bunch) for bunch in networkx.weakly_connected_components(self.graph)
        ]
    ...
    def build_sum_patterns_from_configs(config: Configuration) -> List[dict]:
        ...
        all_patterns = (
            approved_default_patterns + approved_specified_patterns + approved_second_level_specified_patterns
        )
