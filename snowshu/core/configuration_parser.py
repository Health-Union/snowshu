...
from snowshu.configs import DEFAULT_MAX_NUMBER_OF_OUTLIERS, DEFAULT_PRESERVE_CASE, DEFAULT_THREAD_COUNT
...
class ConfigurationParser:
    ...
    def get_dict_from_anything(self, dict_like_object: Union[str, "StringIO", dict], schema_path: Path) -> dict:
    ...
    @staticmethod
    def _verify_schema(instance, file_path: Path, schema_path: Path):
    ...
    @staticmethod
    def _set_default(dict_from: dict, attr: str, default: Any = "") -> None:
    ...
    def _build_general_relations(self, loaded: dict) -> MatchPattern:
        ...
        for database in loaded["source"]["general_relations"]["databases"]:
            ...
            for schema in database["schemas"]:
                ...
                relation_patterns = [
                    MatchPattern.RelationPattern(self.case(relation)) for relation in schema["relations"]
                ]
                schema_patterns.append(MatchPattern.SchemaPattern(schema_pattern, relation_patterns))

            pattern_list.append(MatchPattern.DatabasePattern(database_pattern, schema_patterns))
    ...
    def from_file_or_path(self, loadable: Union[Path, str, TextIO]) -> Configuration:
        ...
        if loaded["source"]["copy_views_as_tables"]:
            materialization_mappings = {"BASE TABLE": materializations.TABLE, "VIEW": materializations.TABLE}
        else:
            materialization_mappings = {"BASE TABLE": materializations.TABLE, "VIEW": materializations.VIEW}
        ...
        try:
            ...
            return Configuration(*replica_base, general_relations, specified_relations)
    ...
    def _build_relationships(self, specified_pattern: dict) -> SpecifiedMatchPattern.Relationships:
        ...
        def build_polymorphic_relationship(sub) -> SpecifiedMatchPattern.PolymorphicRelationshipPattern:
            override_dict = {}
    ...
    def _build_specified_relations(self, source_config: dict) -> SpecifiedMatchPattern:
        ...
        return [
            SpecifiedMatchPattern(
                self.case(rel["database"]),
                self.case(rel["schema"]),
                self.case(rel["relation"]),
                rel.get("unsampled", False),
                sampling_or_none(rel),
                rel.get("include_outliers", None),
                self._build_relationships(rel),
            )
            for rel in specified_relations
        ]

    def _build_adapter_profile(self, section: str, full_configs: Union[str, "StringIO", dict]) -> AdapterProfile:
        profile = full_configs[section]["profile"]
        credentials = full_configs["credpath"]

        def lookup_profile_from_creds(creds_dict: dict, profile: str, section: str) -> dict:
        ...
        try:
            profile_dict = lookup_profile_from_creds(
                self.get_dict_from_anything(credentials, CREDENTIALS_JSON_SCHEMA), profile, section
            )
