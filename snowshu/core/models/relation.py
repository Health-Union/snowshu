...
class Relation:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.materialization == other.materialization
        )
    ...
    def temp_dot_notation(self) -> str:
        ...
        if missing:
            raise ValueError(f"Cannot create temp dot notation. Missing {', '.join(missing)}")
...
def lookup_single_relation(lookup: dict, relation_set: iter) -> Relation:
    ...
    found = next(
        (
            ...
            if rel.database == lookup["database"] and rel.schema == lookup["schema"] and rel.name == lookup["relation"]
        ),
        None,
    )
...
def single_full_pattern_match(
    rel: Relation, pattern: Union[dict, "SpecifiedMatchPattern"], flags: re.RegexFlag = 0
) -> bool:
    ...
    try:
        pattern = dict(database=pattern.database_pattern, schema=pattern.schema_pattern, name=pattern.relation_pattern)
    ...
    return all(
        [
            re.fullmatch(pattern[attr], rel.__dict__[attr], flags)  # noqa pylint: disable=use-a-generator
