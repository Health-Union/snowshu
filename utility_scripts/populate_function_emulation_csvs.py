...
class PGScraper:
    ...
    def get_pg_methods(self):
        ...
        self.pg_methods = dict(
            self.uppers(
                [
                    (
                        sql.text.strip(),
                        f'https://www.postgresql.org/docs/current/{sql["href"]}',
                    )
...
def main():
    ...
    chained = [
        (
            sql.text.strip(),
            f'https://docs.snowflake.net/manuals/sql-reference/{sql.parent["href"]}',
        )
    ...
    for key, value in in_order:
        ...
        row = (
            "`{snowflake_function} <{snowflake_url}>`__," "{postgres_val}," "{emulation_val}," '"{comment_val}"\n'
        ).format(
            snowflake_function=key,
            snowflake_url=value,
            postgres_val=scraper.find_with_url(scraper.pg_methods, key),
            emulation_val=scraper.find_with_url(scraper.snowshu_emulations, key.upper(), 0),
            comment_val=scraper.find_comment(scraper.snowshu_emulations, key.upper(), 1),
        )
        meshed.append(row)
