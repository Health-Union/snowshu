...
def test_creates_replica(docker_flush):
    ...
    for case_name, case_vars in arch_input_options.items():
        ...
        replica_list = shdocker.convert_container_to_replica(test_name_local, target_container, passive_container)
        ...
        for replica in replica_list:
            ...
            client.containers.run(
                replica.id, ports={"9999/tcp": 9999}, name=test_name_local, network="snowshu", detach=True
            )
            time.sleep(DOCKER_SPIN_UP_TIMEOUT)  # give pg a moment to spin up all the way
            engine = create_engine(f"postgresql://snowshu:snowshu@{test_name_local}:9999/snowshu")
