...
INCREMENTAL_CONFIGURATION_PATH = os.path.join(PACKAGE_ROOT, "tests", "assets", "replica_test_incremental_config.yml")
...
def base_for_incremental(docker_flush_module):
    runner = CliRunner()
    runner.invoke(cli, ("create", "--replica-file", INCREMENTAL_CONFIGURATION_PATH, "--multiarch"))
...
def test_base_replica(base_for_incremental):
    ...
    for fake_local_arch in ["amd64", "arm64"]:
        with patch("snowshu.core.main.LOCAL_ARCHITECTURE", fake_local_arch) as _:
            ...
            assert get_tables("integration-test-incremental") == EXPECTED_BASE_TABLE_LIST
...
def test_single_arch_incremental(base_for_incremental):
    ...
    for fake_local_arch in ["amd64", "arm64"]:
        with patch("snowshu.core.main.LOCAL_ARCHITECTURE", fake_local_arch) as _:
            ...
            new_replicas_list = client.images.list("snowshu_replica_integration-test")
            assert len(new_replicas_list) == 1

            # Check if data is fine
            container = client.containers.run(
                f"snowshu_replica_integration-test:{fake_local_arch}",
                ports={"9999/tcp": 9999},
            ...
            assert get_tables("integration-test") == EXPECTED_INREMENTED_TABLE_LIST
            container.remove(force=True)
...
def test_multi_arch_incremental(base_for_incremental):
    ...
    for fake_local_arch in ["arm64", "arm64"]:
        for image_arch in ["amd64", "arm64"]:
            with patch("snowshu.core.main.LOCAL_ARCHITECTURE", fake_local_arch) as _, patch(
                "snowshu.core.docker.LOCAL_ARCHITECTURE", fake_local_arch
            ) as _:
                ...
                if fake_local_arch != image_arch:
                    assert (
                        "Supplied base image is of a non-native architecture, please try to use native for better performance"
                        in output
                    )
                else:
                    assert (
                ...
                for replica in new_replicas_list:
                    container = client.containers.run(
                        replica, ports={"9999/tcp": 9999}, name="integration-test", network="snowshu", detach=True
                    )
                    time.sleep(DOCKER_SPIN_UP_TIMEOUT)
