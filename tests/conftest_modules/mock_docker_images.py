...
class MockImageFactory:
    @staticmethod
    def get_image(
        name: str, is_snowshu: bool = True, source_adapter: str = "snowflake", target_adapter: str = "postgres"
    ) -> MockImage:
        labels = dict()
        if is_snowshu:
        ...
        return MockImage(
            [name], labels, dict(Metadata=dict(LastTagTime=datetime.strftime(random_date(), "%Y-%m-%dT%H:%M:%S.%fZ")))
        )
