...
def test_traverse_and_execute_analyze(stub_graph_set):
    ...
    source_adapter.check_count_and_query.return_value = (
...
def test_traverse_and_execute_custom_max_rows_pass(stub_graph_set):
    ...
    for do_analyze in [True, False]:
        ...
        dag_executable = GraphExecutable(dag, source_adapter, target_adapter, do_analyze)

        with mock.patch.object(
            source_adapter,
            "check_count_and_query",
            return_value=(pd.DataFrame([dict(population_size=1000, sample_size=100)]), 1000),
        ...
        with mock.patch.object(
            ...
            "check_count_and_query",
            return_value=(pd.DataFrame([dict(population_size=1000, sample_size=100)]), 1000),
        ) as mock_2, mock.patch.object(Relation, "data", new=fake_data):
            runner._traverse_and_execute(dag_executable)
