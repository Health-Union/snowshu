...
def test_custom_retry_count_passed_correctly_through_execute(
    graph_to_result_list, printable_result, stub_graph_set, stub_configs
    ...
    with patch.object(SnowShuGraph, "build_graph", new=fake_build_graph), patch.object(
        GraphSetRunner, "execute_graph_set"
    ) as execute_graph_set_mock:
        for do_analyze in [True, False]:
            ...
            execute_graph_set_mock.assert_called_with(
                ANY, ANY, ANY, threads=ANY, retry_count=5, analyze=do_analyze, barf=ANY
            )
