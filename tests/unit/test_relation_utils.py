...
def test_at_least_one_full_pattern_complex():
    test_relation=relation.Relation(database='SNOW_DATABASE',schema="TEST_SCHEMA",name="TEST_RELATION",materialization=TABLE, attributes=[])
...
def test_lookup_single_relation():
    test_relation=relation.Relation(database='SNOW_DATABASE',schema="TEST_SCHEMA",name="TEST_RELATION",materialization=TABLE, attributes=[])
    pattern1=dict(database="(?i)snow_.*",schema=".*",name=".*")
    pattern2=dict(database="SNOW_DATABASE",schema='TEST_SCHEMA',name='TEST_RELATION')
    pattern3=dict(database="NO",schema='banana',name='.*')
    ...
    assert relation.lookup_single_relation(pattern2,[test_relation]) == test_relation

def test_lookup_relations():
    test_relation=relation.Relation(database='SNOW_DATABASE',schema="TEST_SCHEMA",name="TEST_RELATION",materialization=TABLE, attributes=[])
    test_relation2=relation.Relation(database='hamburger',schema="socks",name="TEST_RELATION",materialization=TABLE, attributes=[])
    pattern1=dict(database="(?i)snow_.*",schema=".*",name=".*")
    pattern2=dict(database="SNOW_DATABASE",schema='TEST_SCHEMA',name='TEST_RELATION')
    pattern3=dict(database="NO",schema='banana',name='.*')

    assert relation.lookup_relations(pattern1,[test_relation,test_relation2]) == [test_relation]
    assert not relation.lookup_relations(pattern2,[test_relation,test_relation2]) == [test_relation,test_relation2]
    assert relation.lookup_single_relation(pattern3,[test_relation,test_relation2]) == None
