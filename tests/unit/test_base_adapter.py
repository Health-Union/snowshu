...
from snowshu.core.models.credentials import ACCOUNT, DATABASE, HOST, PASSWORD, ROLE, SCHEMA, USER, Credentials
...
def test_sets_credentials_success(stubbed_adapter, creds_args):
    stubbed_adapter.credentials = rand_creds(creds_args)
    stubbed_adapter.credentials = rand_creds(
        (
            USER,
            PASSWORD,
            HOST,
...
def test_default_conn_string(stubbed_adapter):
    ...
    assert (
        stubbed_adapter._build_conn_string()
        == f"postgres://{creds.user}:{creds.password}@{creds.host}/{creds.database}?account={creds.account}"
    )


@pytest.mark.parametrize(
    "config_patterns, mock_filtered_schema, included_relations, excluded_relations",
    [
        (
            [
            ...
            [
                BaseSQLAdapter._DatabaseObject(
                    "SOURCE_SYSTEM", Relation("snowshu_development", "source_system", "", None, None)
                ),
                BaseSQLAdapter._DatabaseObject(
                    "Cased_Schema", Relation("snowshu_development", "Cased_Schema", "", None, None)
                ),
            ],
            [
...
def test_build_catalog(stubbed_adapter, config_patterns, mock_filtered_schema, included_relations, excluded_relations):
    ...
    with patch.object(StubbedAdapter, "_get_filtered_schemas", return_value=mock_filtered_schema), patch.object(
        StubbedAdapter, "_get_relations_from_database", side_effect=mock_get_relations_func
    ):
