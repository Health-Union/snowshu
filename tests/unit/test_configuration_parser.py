...
def test_schema_verification(tmpdir, stub_creds, stub_configs):
    ...
    replica_config = ConfigurationParser().get_dict_from_anything(replica_file, REPLICA_JSON_SCHEMA)
    cred_config = ConfigurationParser().get_dict_from_anything(cred_file, CREDENTIALS_JSON_SCHEMA)
...
materialization_mappings_test_cases = [
    (
        True,  # set to True
        {"BASE TABLE": materializations.TABLE, "VIEW": materializations.TABLE},
    ),
    (
        False,  # set to False
        {"BASE TABLE": materializations.TABLE, "VIEW": materializations.VIEW},
    ),
    (
        None,  # Not set
        {"BASE TABLE": materializations.TABLE, "VIEW": materializations.TABLE},
