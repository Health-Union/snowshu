...
@pytest.mark.parametrize(
    "sample_method, expected_sampling", [("default", DefaultSampling), ("brute_force", BruteForceSampling)]
)
def test_finds_bruite_force(sample_method, expected_sampling):
    ...
    """
    assert isinstance(get_sampling_from_partial(sample_method), expected_sampling), (
        f"The {expected_sampling.__name__} " f"test case failed."
    )
