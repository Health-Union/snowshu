...
MOCKED_CONFIG = dict(
    ...
    source=dict(
        profile="default",
        sampling="default",
        general_relations=dict(
            databases=[dict(pattern="(?i)^snow.*", schemas=[dict(pattern="THING", relations=[".*suffix$"])])]
        ),
        specified_relations=[
            dict(
                database="(?i)^snow.*",
...
@pytest.mark.skip(
    "TODO This test needs to be redone since the filtering is completed when the catalog is created, not during graph building"
)
