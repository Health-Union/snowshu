...
def tests_replica_rename(_, build_graph, stub_configs):
    ...
    replica.load_config(stub_configs(), target_arch=[LOCAL_ARCHITECTURE.value])
...
def tests_incremental_flag(graph, stub_configs):
    ...
    adapter.initialize_replica.assert_called_once_with(config=replica.config, incremental_image=replica.incremental)
...
def tests_incremental_run_patched(stub_graph_set, stub_relation_set):
    ...
    source_catalog = [
        alter_relation_case(str.upper)(common_relation),
        vals.downstream_relation,
        vals.upstream_relation,
        vals.birelation_right,
    ]
    target_catalog = {
        alter_relation_case(str.lower)(copy.deepcopy(common_relation)),
        vals.downstream_relation,
        vals.upstream_relation,
