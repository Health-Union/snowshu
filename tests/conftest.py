...
from tests.conftest_modules.test_configuration import CONFIGURATION, BASIC_CONFIGURATION, CYCLE_CONFIGURATION  # noqa: F401
...
class RelationTestHelper:
    ...
    def rand_relation_helper(self) -> dict:
        return dict(
            database=rand_string(10).upper(), schema=rand_string(15).upper(), materialization=mz.TABLE, attributes=[]
        )
    ...
    def __init__(self):
        ...
        self.parent_relation_childid_type = Relation(name="PARENT_RELATION_CHILDID_TYPE", **self.rand_relation_helper())
        self.parent_relation_parentid = Relation(name="PARENT_RELATION_PARENTID", **self.rand_relation_helper())
        self.child_relation_type_1 = Relation(name="CHILD_TYPE_1_RECORDS", **self.rand_relation_helper())
        self.child_relation_type_2 = Relation(name="CHILD_TYPE_2_RECORDS", **self.rand_relation_helper())
        self.child_relation_type_3 = Relation(name="CHILD_TYPE_3_RECORDS", **self.rand_relation_helper())
        ...
        self.parent_relation_parentid.attributes = [Attribute(self.parentid_key, dt.VARCHAR)]
...
def stub_graph_set() -> tuple:
    ...
    dag.add_edge(
        ...
        local_attribute=vals.bidirectional_key_right,
        remote_attribute=vals.bidirectional_key_left,
    )
    dag.add_edge(
...
def sanitize_docker_environment():
    ...
    def is_snowshu_related_image(image) -> bool:
        ...
        return any(
            [
                re.search(val, image.tags[0])
                for val in (
                    r"^snowshu_replica_\w+",
                    "integration-test",
                    "snowshu_target",
                )
            ]
