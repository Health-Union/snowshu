...
def get_connection_profile(credentials):
    ...
    if credentials["sources"][0]["database"].upper() != TEST_DATABASE:
        raise ValueError("Test database set in file and database in configurations do not match.")
...
def get_all_csv_file():
    ...
    all_csv_files = [file for path, subdir, files in os.walk(DATA_DIR) for file in glob.glob(os.path.join(path, EXT))]
    ...
    for full_table_name, csv_file in tables.items():
        ...
        with open(csv_file) as f:
            ...
            with engine.begin() as cursor:
                ...
                frame.to_sql(table_name, cursor, schema=schema_name, index=False, chunksize=16000, if_exists="fail")
...
def make_views(engine):
    ...
    users_view = dict(
        name="USERS_VIEW", schema="SOURCE_SYSTEM", sql='SELECT * FROM "SNOWSHU_DEVELOPMENT"."SOURCE_SYSTEM"."USERS"'
    )
